...
const VendorTable = () => {
  ...
  return (
    <div>
      <table className="border-2  border-collapse bg-white shadow-lg rounded-lg overflow-hidden ">
        <tr className=" text-gray-900 text-left ">
          {/* <th className='px-6 py-6 font-medium uppercase tracking-wider'>id</th> */}

          <th className="px-3 py-4  capitalize tracking-wider">businessName</th>
        ...
        <tbody className="whitespace-nowrap ">
          {vendor &&
            vendor.map((vendorItem) => (
              <tr
                className="border-b-2 hover:bg-gray-100 transition-all text-lg sm:text-sm "
                key={vendorItem.id}
              >
                ...
                <td className="px-3 py-4 text-gray-700 ">
                  ...
                  {vendorItem.description.length > 50
                    ? `${vendorItem.description.substring(0, 50)}...`
                ...
                <td
                  className={clsx(
                    'px-3 py-4 text-gray-700 capitalize',
                    vendorItem?.isApproved ? 'text-blue-800' : 'text-red-500',
                  )}
                >
                ...
                <td>
                  <button
                    ...
                    onClick={() => handleChange(vendorItem)}
                  >
                    view
                  </button>
                </td>

                {vendorItem.isApproved === false && (
                  <td>
                    <select
                      name=""
                      id=""
                      className="border-2 px-3 py-2 outline-none rounded-lg"
                      value={selectStatus}
                      onChange={(e) =>
                        handleApprovalSubmission(e, vendorItem.id)
                      }
                    >
                ...
                {/* <td className="px-3 sm:px-4 py-4 text-blue-800">
                  <i class="fa-solid fa-pen"></i>
                </td>
                <td className="px-3 sm:px-4 py-4 text-red-700">
